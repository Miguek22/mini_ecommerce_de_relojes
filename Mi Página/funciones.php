...
function obtenerProductosEnCarrito()
{
    ...
    $sentencia = $bd->prepare("SELECT productos.id, productos.nombre, productos.descripcion, productos.precio, productos.cantidad
     FROM productos
...
{
    $bd = obtenerConexion();
    $sentencia = $bd->query("SELECT * FROM carrito_usuarios");
    return $sentencia->fetchAll();
}
...
function quitarTodoDelCarrito()
{
    $bd = obtenerConexion();
    iniciarSesionSiNoEstaIniciada();
    $idSesion = session_id();
    $sentencia = $bd->prepare("DELETE FROM carrito_usuarios WHERE id_sesion = ?");
    return $sentencia->execute([$idSesion]);
}
function obtenerProductos()
{
    $bd = obtenerConexion();
    $sentencia = $bd->query("SELECT id, nombre, descripcion, precio, cantidad, imagen FROM productos");
    return $sentencia->fetchAll();
}
...
function guardarProducto($nombre, $precio, $cantidad, $descripcion, $ruta)
{
    $bd = obtenerConexion();
    $sentencia = $bd->prepare("INSERT INTO productos(nombre, precio, descripcion, cantidad, imagen) VALUES(?, ?, ?, ?, ?)");
    return $sentencia->execute([$nombre, $precio, $descripcion, $cantidad, $ruta]);
}
